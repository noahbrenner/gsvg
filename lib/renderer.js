...
var getIndentUnit = (function () {
    ...
    return function (indent) {
        ...
        throw new Error(
            '"opt.' + indent + '" must be a positive integer ' +
...
var pushLines = (function () {
    ...
    return function (node, lines) {
        var inlineStart = contains(config.inlineStartNodes, node.name) || (
            node.name === 'closetag' &&
            !contains(config.newlineEndNodes, node.body)
        );
...
var pushSimpleNodes = function (node) {
    var delims = config.delimiters[node.name];
...
var pushOpenTags = function (node) {
    ...
    var delims = node.isSelfClosing ?
        config.delimiters.selfclosingtag :
        config.delimiters.opentag;
...
module.exports = function (tokens, options) {
    ...
    tokens.forEach(function (node) {
        if (contains(config.simpleNodes, node.name)) {
